...
public class PostsController {
    private static final String NAME_ERROR = "Название не должно быть короче двух символов";
    private static final String BODY_ERROR = "Пост должен быть не короче 10 символов";
    ...
    public static void create(Context ctx) {
        try {
            var name = ctx.formParamAsClass("name", String.class)
                .check(value -> value.length() >= 2, NAME_ERROR)
                .get();

            var body = ctx.formParamAsClass("body", String.class)
                .check(value -> value.length() >= 10, BODY_ERROR)
                .get();
